void _unc_mass_ratio()
{
    ...
    TFile *MyFile = new TFile("datas/hist.root", "RECREATE");
    ...
    { //bin_counter-1
        ...
        { //counter-1
            ...
            for (size_t k = 1; k <= Data_NBins; k++)
            {
                ...
                auto Xaxis_val2 = h_pythia[i]->GetBinCenter(k);
            ...
            if (j == 6)
            {
                ...
                for (size_t k = 1; k <= Data_NBins; k++)
                {
                    ...
                    if (check_bins != 0)
                    {
                        ...
                        auto up_shift = sqrt(pow(jec_up, 2) + pow(lumi_unc, 2) + pow(det_unc, 2));
                        auto down_shift = sqrt(pow(jec_down, 2) + pow(lumi_unc, 2) + pow(det_unc, 2));
                ...
                h_Ratio_pythia_up[i]->SetFillColor(BackgroundColor);
                h_Ratio_pythia_down[i]->SetFillColor(kYellow);

                h_Ratio_pythia_down[i]->GetXaxis()->SetRange(xmin, xmax);
                h_Ratio_pythia_down[i]->SetXTitle("M_{ij} (GeV)");
                h_Ratio_pythia_down[i]->SetYTitle("MC/Data");
                h_Ratio_pythia_down[i]->GetYaxis()->CenterTitle();
                ...
                h_Ratio_pythia_down[i]->GetYaxis()->SetRangeUser(0.6, 1.3);
                h_Ratio_pythia_down[i]->GetYaxis()->SetTitleOffset(1.3);
                h_Ratio_pythia_down[i]->GetXaxis()->SetMoreLogLabels();
                h_Ratio_pythia_down[i]->GetXaxis()->SetNoExponent();
                //                h_Ratio_pythia_down[i]->GetXaxis()->SetNdivisions();
                h_Ratio_pythia_down[i]->GetXaxis()->SetTitleOffset(1.3);

                h_Ratio_pythia_up[i]->SetMarkerColor(kWhite);
                h_Ratio_pythia_up[i]->SetLineColor(kWhite);

                h_Ratio_pythia_down[i]->SetMarkerColor(kWhite);
                h_Ratio_pythia_down[i]->SetLineColor(kWhite);

                h_Ratio_pythia_down[i]->Draw("HIST C");
                ...
                leg->AddEntry(h_Ratio_pythia_down[i], "JEC+Lumi+Detector Uncertainty", "lf");
                ...
                if (i == 0)
                {
                    h_trig_uncut_up[i]->Write("eta0_up");
                    h_trig_uncut_down[i]->Write("eta0_down");
                }
                if (i == 1)
                {
                    h_trig_uncut_up[i]->Write("eta1_up");
                    h_trig_uncut_down[i]->Write("eta1_down");
                }
                if (i == 2)
                {
                    h_trig_uncut_up[i]->Write("eta2_up");
                    h_trig_uncut_down[i]->Write("eta2_down");
                }
                if (i == 3)
                {
                    h_trig_uncut_up[i]->Write("eta3_up");
                    h_trig_uncut_down[i]->Write("eta3_down");
                }
                if (i == 4)
                {
                    h_trig_uncut_up[i]->Write("eta4_up");
                    h_trig_uncut_down[i]->Write("eta4_down");
                }
