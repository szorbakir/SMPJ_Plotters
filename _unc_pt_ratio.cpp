void _unc_pt_ratio()
{
    ...
    constexpr size_t counter = (sizeof(observable) / sizeof(*observable));
    constexpr size_t bin_counter = (sizeof(eta_bins) / sizeof(*eta_bins));
    ...
    for (size_t j = 0; j < bin_counter - 1; j++)
    {
        for (size_t i = 0; i < counter; i++)
        {
            ...
            h_Ratio_PYT_up_->SetFillColor(BackgroundColor);
            h_Ratio_PYT_down_->SetFillColor(kYellow);
            ...
            //cosmetics
            h_Ratio_PYT_down_->GetXaxis()->SetRangeUser(ymin, 2500);
            h_Ratio_PYT_down_->SetXTitle(axis_name[i]);
            h_Ratio_PYT_down_->SetYTitle("MC/Data");
            h_Ratio_PYT_down_->GetYaxis()->CenterTitle();
            ...
            h_Ratio_PYT_down_->GetYaxis()->SetRangeUser(0.6, 1.6);
            h_Ratio_PYT_down_->GetYaxis()->SetTitleOffset(1.3);
            h_Ratio_PYT_down_->GetXaxis()->SetMoreLogLabels();
            h_Ratio_PYT_down_->GetXaxis()->SetNoExponent();
            h_Ratio_PYT_down_->GetXaxis()->SetTitleOffset(1.3);

            h_Ratio_PYT_down_->SetLineColor(kWhite);
            h_Ratio_PYT_down_->SetMarkerColor(kWhite);

            h_Ratio_PYT_up_->SetLineColor(kWhite);
            h_Ratio_PYT_up_->SetMarkerColor(kWhite);

            h_Ratio_PYT_down_->Draw("HIST C");
            ...
            leg->AddEntry(h_Ratio_PYT_down_, "JEC+Lumi+Detector Uncertainty", "lf");
